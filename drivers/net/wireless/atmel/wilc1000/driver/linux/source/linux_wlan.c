...
static int linux_wlan_init_test_config(struct net_device *dev, linux_wlan_t* p_nic){
	...
	*(int*)c_val = (WILC_Uint32)pstrWFIDrv;
	...
	/*to tell fw that we are going to use PC test - ATMEL specific*/
	c_val[0] = 0;
	...
	**/
	...
	if(wilc_get_chipid(WILC_FALSE) >= 0x1002a0)
		c_val[0] = 7; /* Limit TX rate below MCS-7 for WILC1000F0. Should be changed in to 7 in NMC1002. */
	else
		c_val[0] = 6; 
...
/*	c_val[0] = 1;
	...
	if (!g_linux_wlan->oup.wlan_cfg_set(0, WID_RESET, c_val, 1, 1,(WILC_Uint32)pstrWFIDrv))
		goto _fail_;
*/
...
void wilc1000_wlan_deinit(linux_wlan_t *nic) {
	...
	if(g_linux_wlan->wilc1000_initialized)
	{
		...
		PRINT_D(INIT_DBG,"Deinitializing wilc1000  ...\n");
	...
	PRINT_D(INIT_DBG,"Deinitializing ATMEL Wlan\n");
...
int mac_open(struct net_device *ndev){    
	...
	//{
		...
		host_int_set_MacAddress(priv->hWILCWFIDrv, mac_add);
	...
	//if(!once)
     //host_int_set_wfi_drv_handler((WILC_Uint32)priv->hWILCWFIDrv);
   //host_int_set_wfi_drv_handler((WILC_Uint32)priv->hWILCWFIDrv_2);
...
*/
	...
	//saddr = &ih->saddr;
	//daddr = &ih->daddr;
	//WILC_PRINTF("Source IP = %d:%d:%d:%d \n",saddr[0],saddr[1],
		//	saddr[2],saddr[3]);
//	WILC_PRINTF("Dest IP = %d:%d:%d:%d \n",daddr[0],daddr[1],
	//			daddr[2],daddr[3]);
	...
	WILC_PRINTF("Source MAC = %.2x:%.2x:%.2x:%.2x:%.2x:%.2x \n",ethsaddr[0],ethsaddr[1],
			ethsaddr[2],ethsaddr[3],ethsaddr[4],ethsaddr[5]);
	WILC_PRINTF("Dest MAC = %.2x:%.2x:%.2x:%.2x:%.2x:%.2x \n",ethdaddr[0],ethdaddr[1],
			ethdaddr[2],ethdaddr[3],ethdaddr[4],ethdaddr[5]);*/
     
    //WILC_PRINTF("sending pkt \n");

   // WILC_PRINTF("skb->data[12] = %2x, skb->data[13] = %2x \n");
...
#ifdef USE_WIRELESS
			WILC_WFI_DeInitHostInt(g_linux_wlan->strInterfaceInfo[i].nmc_netdev);
#endif
		}
...
void WILC_WFI_mgmt_rx(uint8_t *buff, uint32_t size)
{
	...
	for(i = 0;i< g_linux_wlan->u8NoIfcs;i++)
	{
		...
		{
			WILC_WFI_p2p_rx(g_linux_wlan->strInterfaceInfo[i].nmc_netdev,buff,size);
		}
		#endif
	}
...
int nmc_netdev_init(void){
	...
	for(i=0;i<NUM_CONCURRENT_IFC;i++)	
	{
		...
		#ifdef USE_WIRELESS
		{
			...
			if(wdev == NULL){
			PRINT_ER("Can't register ATMEL Wiphy\n");
			return -1;
			}
