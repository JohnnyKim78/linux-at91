...
#include "linux/include/wilc_wfi_cfgoperations.h"
#include "wilc_wlan.c"
...
WILC_Uint8 u8WLANChannel = DEFAULT_CHANNEL ;
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if(nic->iftype == STATION_MODE)
	{
		...
		#if 0
		WILC_Sint8 s8lnkspd = 0;
		static WILC_Uint32 u32FirstLnkSpdCnt = 0;

		
		sinfo->filled |= STATION_INFO_SIGNAL;

		s32Error = host_int_get_rssi(priv->hWILCWFIDrv, &(sinfo->signal));
		if(s32Error)
		...
		{
			priv->gbAutoRateAdjusted = WILC_TRUE;
		}

		if(priv->gbAutoRateAdjusted == WILC_FALSE)
		{
			sinfo->txrate.legacy = 54 * 10;
		}
		else
		{	
			s32Error = host_int_get_link_speed(priv->hWILCWFIDrv, &s8lnkspd);
			if(s32Error)
			{
				PRINT_ER("Failed to send get host channel param's message queue ");
			}
...
*  @version
*/
void WILC_WFI_CfgParseTxAction(WILC_Uint8 * buf,WILC_Uint32 len,WILC_Bool bOperChan)
{
	WILC_Uint32 index=0;
	WILC_Uint32 i=0,j=0;
...
void WILC_WFI_p2p_rx (struct net_device *dev,uint8_t *buff, uint32_t size)
{
	...
	{	
				...
				case GAS_INTIAL_REQ:
					...
					case PUBLIC_ACT_VENDORSPEC:
					{
								...
								if(!bWilc_ie)
								{
									for(i=P2P_PUB_ACTION_SUBTYPE;i<size;i++)
									{
										if(!WILC_memcmp(u8P2P_vendorspec,&buff[i],6))
										{
											...
											bWilc_ie=WILC_TRUE;
											PRINT_D(GENERIC_DBG,"ATMEL Vendor specific IE:%02x\n",u8P2Precvrandom);
											break;
										}
									}		
								}
						...
						if( (buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_REQ ||  buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_RSP ) && ( bWilc_ie))
						{
							...
							cfg80211_rx_mgmt(dev,priv->u32listen_freq, 0, buff,size-7,GFP_ATOMIC);
							#else
							cfg80211_rx_mgmt(priv->wdev,priv->u32listen_freq, 0, buff,size-7,GFP_ATOMIC);
							#endif
		...
		#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3,6,0))
		cfg80211_rx_mgmt(dev,priv->u32listen_freq, 0, buff,size,GFP_ATOMIC);
		#else
		cfg80211_rx_mgmt(priv->wdev,priv->u32listen_freq, 0, buff,size,GFP_ATOMIC);
		#endif
	}
}
...
static void WILC_WFI_RemainOnChannelExpired(void* pUserVoid)
{	
	struct WILC_WFI_priv* priv;
	priv = (struct WILC_WFI_priv*)pUserVoid;	
	...
	unsigned int duration,u64 *cookie)
{
	...
	freq = ieee80211_channel_to_frequency((WILC_Sint32)chan->hw_value);
	#else
	freq = ieee80211_channel_to_frequency((WILC_Sint32)chan->hw_value, IEEE80211_BAND_2GHZ);
        #endif
	priv->u32listen_freq= freq;

	s32Error = host_int_remain_on_channel(priv->hWILCWFIDrv, duration,chan->hw_value,WILC_WFI_RemainOnChannelExpired,WILC_WFI_RemainOnChannelReady,(void *)priv);
...
*  @details 	Adding ATMEL information elemet to allow two ATMEL devices to
	...
	*cookie = (unsigned long)buf;
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) 
	{
		...
		else if (ieee80211_is_action(mgmt->frame_control)) 
		{
			...
			{
				WILC_PRINTF("PUBLIC ACTION FRAME\n");
				...
				switch(buf[ACTION_SUBTYPE_ID])
				{
					...
					case PUBLIC_ACT_VENDORSPEC:
					{
						...
						{
							/*For the connection of two ATMEL's connection generate a rand number to determine who will be a GO*/
									...
									for(i=P2P_PUB_ACTION_SUBTYPE+2;i<len;i++)
									{
								       	if(buf[i]== P2PELEM_ATTR_ID && !(WILC_memcmp(u8P2P_oui,&buf[i+2],4)) )
										{
											if(buf[P2P_PUB_ACTION_SUBTYPE]==P2P_INV_REQ || buf[P2P_PUB_ACTION_SUBTYPE]==P2P_INV_RSP)
												WILC_WFI_CfgParseTxAction(&mgmt_tx->buff[i+6],len-(i+6),WILC_TRUE); 
											else
												WILC_WFI_CfgParseTxAction(&mgmt_tx->buff[i+6],len-(i+6),WILC_FALSE); 
											break;
										}
									}
	...
	enum nl80211_iftype type, u32 *flags,struct vif_params *params)
{
	...
	case NL80211_IFTYPE_P2P_CLIENT:
		...
		host_int_delBASession(priv->hWILCWFIDrv, g_linux_wlan->strInterfaceInfo[0].aBSSID, TID, (void *)priv->hWILCWFIDrv);
