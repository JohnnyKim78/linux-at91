...
static int sdio_init(wilc_wlan_inp_t *inp, wilc_debug_func func)
{
	...
	**/
	...
	g_sdio.dPrint(N_ERR, "[wilc sdio]: has_thrpt_enh = %d...\n", g_sdio.has_thrpt_enh);
	g_sdio.dPrint(N_ERR, "[wilc sdio]: has_thrpt_enh2 = %d...\n", g_sdio.has_thrpt_enh2);
	g_sdio.dPrint(N_ERR, "[wilc sdio]: reg_0xf6_not_clear_bug_fix = %d...\n", g_sdio.reg_0xf6_not_clear_bug_fix);
#endif
...
static int sdio_write_ext(uint32_t addr, uint8_t *buf, uint32_t size)
{
	...
	if(g_sdio.reg_0xf6_not_clear_bug_fix) {/* bug 4456 and 4557 */
		if(g_sdio.has_thrpt_enh) {
			...
			cmd.data = 0;	
			if(!g_sdio.sdio_cmd52(&cmd)) {
				g_sdio.dPrint(N_ERR, "[wilc sdio]: Fail cmd 52, set 0xf6 register (%d) ...\n", __LINE__);
				goto _fail_;
			}
		}
	}
...
static int sdio_read_size(uint32_t * size)
{
	...
	**/	
	if (g_sdio.has_thrpt_enh) {
		...
		sdio_read_reg(WILC_VMM_TO_HOST_SIZE, &byte_cnt); /* in IRQN interrupt case, read DMA count over AHB. */ 
		...
		do {
			...
			
			{
				...
				if( (tmp >> IRG_FLAGS_OFFSET) & unkmown_mask) {
					g_sdio.dPrint(N_ERR, "[wilc sdio]: Unexpected interrupt (2): j=%d, tmp=%x, mask=%x\n", j, tmp, unkmown_mask);
					happended = 1;
				}
			}
			j++;
...
static int sdio_clear_int_ext(uint32_t val)
{
	...
	if (g_sdio.has_thrpt_enh2) {
		...
		if(flags) {
			...
			if (!ret) {
				g_sdio.dPrint(N_ERR, "[wilc sdio]: Failed sdio_write_reg, set reg %x ...\n", 0x10c8+i*4);
				goto _fail_;
			}
			for(i=g_sdio.nint; i<MAX_NUM_INT; i++) {
				if(flags & 1) g_sdio.dPrint(N_ERR, "[wilc sdio]: Unexpected interrupt cleared %d...\n", i);
	...
	if(g_sdio.has_thrpt_enh) {
		...
		if(vmm_ctl) {	
			...
			if(tbl_ctl) { /* fix bug 4588 */
				ret = sdio_write_reg(WILC_VMM_TBL_CTL, tbl_ctl);
				if (!ret) {
					g_sdio.dPrint(N_ERR, "[wilc sdio]: fail write reg vmm_tbl_ctl...\n");
					goto _fail_;
				}
			}
			...
			if(!tbl_ctl) { /* fix bug 4588 */
				ret = sdio_write_reg(WILC_VMM_CORE_CTL, 1);
				if (!ret) {
					g_sdio.dPrint(N_ERR, "[wilc sdio]: fail write reg vmm_core_ctl...\n");
					goto _fail_;
				}
			} else {
				uint32_t data;
				...
				/**
					Write WILC_VMM_CORE_CTL the way below to save 2 CMD 52. 
					The code should look like #if 0'ed code below. After writing 
					WILC_VMM_TBL_CTL, then CSA address is now WILC_VMM_TBL_CTL+4,
				...
				**/
				...
				/**
					set the AHB address
				**/
				if (!sdio_set_func0_csa_address_byte0(WILC_VMM_CORE_CTL)) 
					goto _fail_;
				...
				cmd.buffer = (uint8_t *)&data;
				if (!g_sdio.sdio_cmd53(&cmd)) {
					g_sdio.dPrint(N_ERR, "[wilc sdio]: Failed cmd53, write WILC_VMM_CORE_CTL...\n");
					goto _fail_;
				}	
			}
		}
