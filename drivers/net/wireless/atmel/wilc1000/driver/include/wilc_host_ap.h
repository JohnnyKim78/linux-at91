...
#include "../linux/include/wilc_wfi_netdevice.h"

#ifdef WILC_FULLY_HOSTING_AP
/*!  
*  @file	WILC_host_AP.h
*  @brief	code related to AP fully hosting mode on ATMEL driver
...
#define rMAC_DTIM_COUNT_ADDR						(( WILC_Uint32 )(PA_BASE + 0x021C))
#define rMAC_BEACON_PERIOD           					(( WILC_Uint32 )(PA_BASE + 0x020C))
#define rMAC_DTIM_PERIOD              					(( WILC_Uint32 )(PA_BASE + 0x0210))
#define rMAC_TSF_CON                 				 		(( WILC_Uint32 )(PA_BASE + 0x0200))
...
#define NEXT_ELEMENT_ADDR(base, ofst) (*((WILC_Uint32 *)((WILC_Uint8 *)base + ofst)))
...
typedef struct
{
    ...
    WILC_Uint16 count;           /* Number of Elements in the List              */
    WILC_Uint16 lnk_byte_ofst;   /* Offset to Link Pointer in the List in Bytes */
} list_buff_t;
...
typedef struct
{
    WILC_Uint8 rates[MAX_RATES_SUPPORTED];
    WILC_Uint8 num_rates;
} rate_t;
...
typedef struct {
	WILC_Uint16		u16beacon_len;
	WILC_Uint8             *u8beacon_frame;
	WILC_Uint8		u8tim_element_index;
	WILC_Uint16      	u16tim_element_trailer_len;
	WILC_Uint8       	u8vbmap[VBMAP_SIZE];
	WILC_Uint8             	u8DTIMPeriod;
	WILC_Uint16            u16Beacon_Period;
}beacon_info;

typedef struct
{
    WILC_Uint8   service_class; /* Service class extracted from header       */
    WILC_Uint8   priority_val;  /* Priority value extracted from header      */
    WILC_Uint8   u8hdr_len;       /* Length of header including offset         */
    WILC_Uint16  u16data_len;      /* Length of the frame excluding headers     */
    WILC_Uint16  u16rx_len;        /* Length of the frame including the headers */
    WILC_Uint8   *u8sa_entry;     /* Source Address station entry              */
    WILC_Uint8   *u8msa;          /* MAC header start address                  */
    WILC_Uint8   u8addr1[6];      /* Address1 field extracted from header      */
    WILC_Uint8   u8addr2[6];      /* Address2 field extracted from header      */
    WILC_Uint8   u8addr3[6];      /* Address3 field extracted from header      */
    WILC_Uint8   *u8sa;           /* Pointer to source address                 */
    WILC_Uint8   *u8da;           /* Pointer to destination address            */
    WILC_Uint8   *u8bssid;        /* Pointer to BSSID                          */
    WILC_Uint8   *u8ta;           /* Pointer to Transmitter address            */
    BASICTYPE_T enumType;       /* Packet type extracted from the header     */
    WILC_Uint8   u8Sub_type;      /* Packet Sub type extracted from the header */
    WILC_Uint8   u8frm_ds;        /* From DS field of MAC header               */
    WILC_Uint8   u8to_ds;         /* To DS field of MAC header                 */
    WILC_Bool 	bIs_grp_addr;   /* Group address flag                        */
} wlan_rx_t;
...
typedef struct   {
  ...
  WILC_Uint8 			addr[ETH_ALEN];				/* MAC address of the STA           */
  WILC_Uint16 			u16AID;          	 			/* Association ID of the STA           */
  STATIONSTATE_T 	state;             				/* Auth/Asoc state of the STA          */
  struct ieee80211_sta_ht_cap ht_cap;
  //struct ieee80211_sta_vht_cap vht_cap;
  WILC_Uint32        		aging_cnt;         				/* Aging cnt for STA                   */
  ...
  WILC_Uint16        num_ps_pkt;        					/* Number of UC pkts queued in PSQ     */
  WILC_Uint16        num_qd_pkt;        					/* Number of UC pkts queued in HW      */
  WILC_Bool	ps_poll_rsp_qed;   					/* Flag to indicate PS poll rsp is     */
...
typedef struct
{
    ...
    WILC_Uint16        listen_interval;   /* Listen Interval of the STA          */
...
{
    WILC_Uint8                key[6];          /* Key, i.e. the MAC address      */
...
extern linux_wlan_t* g_linux_wlan;
 ...
 WILC_Sint32 host_add_beacon(WILC_WFIDrvHandle hWFIDrv, WILC_Uint32 u32Interval,
									 WILC_Uint32 u32DTIMPeriod,
									 WILC_Uint32 u32HeadLen, WILC_Uint8* pu8Head,
									 WILC_Uint32 u32TailLen, WILC_Uint8* pu8Tail);

 WILC_Sint32 host_del_beacon(WILC_WFIDrvHandle hWFIDrv);
...
void WILC_Process_rx_frame(WILC_Uint8 *u8Buff, WILC_Uint32 u32Size);

int WILC_Xmit_data(void* ptx_data, Data_Interface InterfaceType);

WILC_Bool filter_monitor_data_frames(WILC_Uint8 *buf, WILC_Uint16 len);
