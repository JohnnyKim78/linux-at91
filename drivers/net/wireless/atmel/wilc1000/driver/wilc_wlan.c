...
static uint32_t WILC_INT_STATS = 0;
static uint32_t WILC_GP_REG_1 = 0;
...
static int wilc_wlan_handle_txq(void)
{
	wilc_wlan_dev_t *p = (wilc_wlan_dev_t *)&g_wlan;
	...
	do {
		...
		reg |= (1<<1);
		ret = p->hif_func.hif_write_reg(WILC_HOST_VMM_CTL, reg);
		if (!ret) {
		...
		do {
			...
			if (!(reg & 0x2)) {
				ret = p->hif_func.hif_read_reg(WILC_VMM_TX_TBL_BASE, &dma_addr);
				break;
			} 
		} while(1);
		...
		if (!ret) {
			wilc_debug(N_ERR, "[wilc txq]: fail can't read reg vmm_tx_tbl_base...\n");
			release_bus(RELEASE_ALLOW_SLEEP);
			break;
		}
		release_bus(RELEASE_ALLOW_SLEEP);
		...
		if (dma_addr != 0) {
			...
			if ((p->io_func.io_type & 0x1) == HIF_SPI) {
				if (tqe->type) {
					dma_addr += WILC_AHB_DATA_MEM_BASE;
				} else {
					dma_addr += WILC_AHB_SHARE_MEM_BASE;
				}
			...
			if (ret) {
				...
				wilc_debug(N_ERR, "[wilc txq]: fail block tx...\n");
				break;
			}
		} else {
...
static int wilc_wlan_handle_txq(uint32_t* pu32TxqCount)
{
	...
	/**
			...
			**/
			reg = 0x1;
			ret = p->hif_func.hif_write_reg(WILC_VMM_TBL_CTL, reg);
			if (!ret) {
				wilc_debug(N_ERR, "[wilc txq]: fail can't write reg vmm_tbl_ctl...\n");
				goto _end_;
			}
			...
			**/
			...
			if (!ret) {
				wilc_debug(N_ERR, "[wilc txq]: fail can't read reg vmm_core_ctl...\n");
				goto _end_;
			}
			reg |= 0x1;		
			ret = p->hif_func.hif_write_reg(WILC_VMM_CORE_CTL, reg);
			if (!ret) {
				wilc_debug(N_ERR, "[wilc txq]: fail can't write reg vmm_core_ctl...\n");
				goto _end_;
			}
	#else
			reg = 0x1;
			...
			**/
			...
			if (!ret) 
				wilc_debug(N_ERR, "[wilc txq]: fail can't block tx...\n");
	}
_end_:	 
...
********************************************/
static void wilc_unknown_isr(void){
	g_wlan.hif_func.hif_clear_int();
	linux_wlan_enable_irq();
	release_bus(RELEASE_ALLOW_SLEEP);
}
static void wilc_pllupdate_isr(uint32_t int_stats1){
#if (defined WILC_SDIO) && (!defined WILC_SDIO_IRQ_GPIO)

	
	//g_wlan.hif_func.hif_write_reg(WILC_INT_STATS,(int_stats1 & ~PLL_INT));	
	g_wlan.hif_func.hif_write_reg(WILC_INT_STATS,0);	
	...
	g_wlan.hif_func.hif_read_reg(WILC_INT_STATS,&int_stats);	
	g_wlan.hif_func.hif_write_reg(WILC_INT_STATS,(int_stats & ~PLL_INT));	
...
static void wilc_sleeptimer_isr(uint32_t int_stats1){
	uint32_t int_stats;
	...
	g_wlan.hif_func.hif_read_reg(WILC_INT_STATS,&int_stats);	
	g_wlan.hif_func.hif_write_reg(WILC_INT_STATS,(int_stats & ~SLEEP_INT));	
	/* Wait 1ms tomake sure chip went into sleep
	   Accessing the clockless registers in the transit state fails */
	if(wilc_get_chipid(WILC_FALSE) < 0x1000D0)
	{
		WILC_SleepMicrosec(1200); // failed at 1 ms
	}
#ifndef WILC_OPTIMIZE_SLEEP_INT
...
#ifdef WILC1000_SINGLE_TRANSFER
static void wilc_wlan_handle_isr(uint32_t int_status1)
{
	wilc_wlan_dev_t *p = (wilc_wlan_dev_t *)&g_wlan;
#ifdef MEMORY_STATIC
	...
	**/
	int_stats1 &= ~(DATA_INT);
	if(p->hif_func.hif_write_reg(WILC_INT_STATS,int_stats1)!= 1)
	...
	/**
		read the rx size
	**/
	ret = p->hif_func.hif_read_reg(WILC_HOST_RX_CTRL_0, &ctl0);
	size = (ctl0 >> 2) & 0xfff;

	if (!ret) {
		wilc_debug(N_ERR, "[wilc isr]: fail can't read reg host_rx_ctrl_0...\n");
		release_bus(RELEASE_ALLOW_SLEEP);
		return;
	} 
	...
	if (size > 0) {
		...
		**/
		if ((p->io_func.io_type & 0x1) == HIF_SPI) {
			...
			**/
			ret = p->hif_func.hif_read_reg(WILC_HOST_RX_CTRL_1, &address);
			if (!ret) {
				wilc_debug(N_ERR, "[wilc isr]: fail can't read reg host_rx_ctrl_1...\n");
				goto _end_;
			}
			if (address & 0x1) {
				address = WILC_AHB_DATA_MEM_BASE+(address&~0x1);
			} else {
				address = WILC_AHB_SHARE_MEM_BASE+address;	
			}
		} else if ((p->io_func.io_type & 0x1) == HIF_SDIO) {
			address = 0;
		}
		ret = p->hif_func.hif_block_rx(address, buffer, size);
 		if (!ret) {
			...
			if ((p->io_func.io_type & 0x1) == HIF_SPI) {
				...
				**/
				ctl0 |= (1<<1);
				ret = p->hif_func.hif_write_reg(WILC_HOST_RX_CTRL_0, ctl0);
			}
		}
		...
		if (ret) {
			...
			**/
#if 0
			{
				...
				wilc_debug(N_ERR, "*** Receive Packet ***\n"); 

				for (i = 0; i < size; i++) {
					if ((i!=0) &&((i%8)==0))
						wilc_debug(N_ERR, "\n", bt[i]);
					wilc_debug(N_ERR, "%02x ", bt[i]);
				}
				wilc_debug(N_ERR, "\n");
			}
#endif
...
#else
#if defined WILC_SDIO && !defined WILC_SDIO_IRQ_GPIO
static void wilc_wlan_handle_isr(uint32_t xfer_cnt, uint32_t int_stats1)
#else
#ifdef PLL_WORKAROUND
static void wilc_wlan_handle_isr(uint32_t int_stats1)
#else /* PLL_WORKAROUND */
static void wilc_wlan_handle_isr(void)
...
#endif
{
	...
	/**
		clear the chip's interrupt		
	**/
#if defined PLL_WORKAROUND && ( (!defined WILC_SDIO) || (defined WILC_SDIO_IRQ_GPIO))
	int_stats1 &= ~(DATA_INT);
	p->hif_func.hif_write_reg(WILC_INT_STATS,int_stats1);
...
#elif (!defined PLL_WORKAROUND) &&  ( (!defined WILC_SDIO) || (defined WILC_SDIO_IRQ_GPIO))
	p->hif_func.hif_clear_int();	
	linux_wlan_enable_irq();
#endif
	...
	**/
		...
		size = xfer_cnt;	
	#else
		ret = p->hif_func.hif_read_reg(WILC_VMM_TO_HOST_SIZE, &size);
		if (!ret) {
			wilc_debug(N_ERR, "[wilc isr]: fail can't read reg vmm_to_host_size...\n");
			goto _end_;
		} 
	#endif
	...
	if (size > 0) {
		...
		**/
		...
		ret = p->hif_func.hif_write_reg(WILC_VMM_TBL_CTL, reg);
		if (!ret) {
			wilc_debug(N_ERR, "[wilc isr]: fail write reg vmm_tbl_ctl...\n");
			goto _end_;
		}
		...
		**/
		...
		ret = p->hif_func.hif_read_reg(WILC_VMM_CORE_CTL, &reg);
		if (!ret) {
			wilc_debug(N_ERR, "[wilc isr]: fail read reg vmm_core_ctl...\n");
			goto _end_;
		}
		reg |= 0x1;		
		ret = p->hif_func.hif_write_reg(WILC_VMM_CORE_CTL, reg);
		if (!ret) {
			wilc_debug(N_ERR, "[wilc isr]: fail write reg vmm_core_ctl...\n");
			goto _end_;
		}
#else
		reg = 0x1;
...
void wilc_handle_isr_v1(void)
{
		...
		if(xfer_cnt == 0){
			g_wlan.hif_func.hif_read_reg(WILC_INT_STATS,&int_status);
			if(int_status & PLL_INT){
				wilc_pllupdate_isr(int_status);
			}
			if(int_status & SLEEP_INT){
				wilc_sleeptimer_isr(int_status);
			}
		}
		if(xfer_cnt>0){
	#ifndef WILC_OPTIMIZE_SLEEP_INT
			...
			genuChipPSstate = CHIP_WAKEDUP;
	#endif
			wilc_wlan_handle_isr(xfer_cnt, 0);
		}
#else /* ! WILC_SDIO */
	#ifdef PLL_WORKAROUND
	uint32_t int_status;
	...
	if(int_status & DATA_INT){ 
		...
		genuChipPSstate = CHIP_WAKEDUP;
	#endif
		wilc_wlan_handle_isr(int_status);
	}
...
#if DMA_VER == DMA_VER_2
	if(p->use_dma_v2) {
		reg |= WILC_HAVE_DMA_VER_2;
	}
#endif

#ifdef VCO_14_SUPPLY
	if(wilc_get_chipid(WILC_FALSE) < 0x1002a0){
		reg |= WILC_HAVE_VCO_14_SUPPLY;
	}
#endif
	
#ifdef WILC_HOST_HAS_RTC
	reg |= WILC_HAVE_RTC;
#endif
...
#ifdef XTAL_24
	reg |= WILC_XTAL_24;
	wilc_debug(N_ERR, "[wilc start]: set xtal 24Mhz\n");
#else
	reg &= ~WILC_XTAL_24;
	wilc_debug(N_ERR, "[wilc start]: set xtal 26Mhz\n");
#endif

/*BugID_5271*/
/*Enable/Disable GPIO configuration for FW logs*/
#ifdef DISABLE_NMC_UART
	reg |= WILC_DISABLE_NMC_UART;
#else
	reg &= ~WILC_DISABLE_NMC_UART;
#endif
...
static void wilc_wlan_cleanup(void)
{
	...
	**/
	...
	ret = p->hif_func.hif_read_reg(WILC_INT_STATS,&reg); 
	if (!ret) {
		PRINT_ER("Error while reading reg\n");
		release_bus(RELEASE_ALLOW_SLEEP);
	}
	PRINT_ER("Writing ABORT reg\n");
  	ret = p->hif_func.hif_write_reg(WILC_INT_STATS,(reg | ABORT_INT ));  
	if (!ret) {
		PRINT_ER("Error while writing reg\n");
		release_bus(RELEASE_ALLOW_SLEEP);
	}
...
static int wilc_wlan_cfg_set(int start, uint32_t wid, uint8_t *buffer, uint32_t buffer_size, int commit,uint32_t drvHandler)
{
	...
	if (commit) {
		PRINT_D(TX_DBG,"[ATMEL]PACKET Commit with sequence number %d\n",p->cfg_seq_no);
...
static int wilc_wlan_cfg_get(int start, uint32_t wid, int commit,uint32_t drvHandler)
{
	...
	if (commit) {
		...
		PRINT_D(GENERIC_DBG, "[ATMEL]Get Response received\n");
		p->cfg_frame_in_use = 0;
		p->cfg_frame_offset = 0;
		p->cfg_seq_no += 1;
	}
...
uint32_t init_chip(void)
{
	...
	if((chipid & 0xfff) >= 0xd0) {		
		WILC_INT_STATS = _WILC_INT_STATS_D0_AND_LATER;
		WILC_GP_REG_1 = _WILC_GP_REG_1_D0_AND_LATER;
	} else {
		WILC_INT_STATS = _WILC_INT_STATS_PRE_D0;
		WILC_GP_REG_1 = _WILC_GP_REG_1_PRE_D0;
	}
